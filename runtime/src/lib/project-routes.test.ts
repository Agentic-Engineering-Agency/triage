...
async function loadIntegrationRoutes() {
  return await import('./integration-routes');
}
...
describe('data isolation between projects', () => {
  ...
  it('saving a Linear token on project A does not populate project B', async () => {
    seedProject({ id: 'A' });
    seedProject({ id: 'B' });
    const { testLinearTokenRoute } = await loadIntegrationRoutes();
    const ctx = makeCtx({ params: { projectId: 'A' }, body: { token: 'lin_A' } });
    await testLinearTokenRoute.handler(ctx);
    expect(store.projects.get('A')!.linear_token).toBe('lin_A');
    expect(store.projects.get('B')!.linear_token).toBe(null);
  });
...
describe('integration-routes', () => {
  describe('POST /projects/:projectId/settings/linear/test', () => {
    it('returns 400 when token is missing', async () => {
      seedProject({ id: 'p1' });
      const { testLinearTokenRoute } = await loadIntegrationRoutes();
      const res = (await testLinearTokenRoute.handler(
        makeCtx({ params: { projectId: 'p1' }, body: {} }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(400);
      expect((res.body as { error: { code: string } }).error.code).toBe('MISSING_TOKEN');
    });

    it('returns 401 when Linear viewer throws (invalid token)', async () => {
      seedProject({ id: 'p1' });
      linearMockState.viewerThrows = true;
      const { testLinearTokenRoute } = await loadIntegrationRoutes();
      const res = (await testLinearTokenRoute.handler(
        makeCtx({ params: { projectId: 'p1' }, body: { token: 'bad' } }),
      )) as unknown as JsonResponse;
    ...
    it('returns 404 when project does not exist', async () => {
      const { testLinearTokenRoute } = await loadIntegrationRoutes();
      const res = (await testLinearTokenRoute.handler(
        makeCtx({ params: { projectId: 'ghost' }, body: { token: 'ok' } }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(404);
    });

    it('saves the token on success and returns user', async () => {
      seedProject({ id: 'p1' });
      const { testLinearTokenRoute } = await loadIntegrationRoutes();
      const res = (await testLinearTokenRoute.handler(
        makeCtx({ params: { projectId: 'p1' }, body: { token: 'good' } }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(200);
      expect(store.projects.get('p1')!.linear_token).toBe('good');
      const body = res.body as { data: { user: { id: string } } };
      expect(body.data.user.id).toBe('u1');
    });
  });

  describe('POST /projects/:projectId/settings/linear/webhook', () => {
    it('returns 400 when url is missing', async () => {
      seedProject({ id: 'p1', linear_token: 'x' });
      const { registerLinearWebhookRoute } = await loadIntegrationRoutes();
      const res = (await registerLinearWebhookRoute.handler(
        makeCtx({ params: { projectId: 'p1' }, body: {} }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(400);
      expect((res.body as { error: { code: string } }).error.code).toBe('MISSING_URL');
    });

    it('returns 404 when project not found', async () => {
      const { registerLinearWebhookRoute } = await loadIntegrationRoutes();
      const res = (await registerLinearWebhookRoute.handler(
        makeCtx({ params: { projectId: 'ghost' }, body: { url: 'https://x' } }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(404);
    });

    it('returns 400 when project lacks a Linear token', async () => {
      seedProject({ id: 'p1', linear_token: null });
      const { registerLinearWebhookRoute } = await loadIntegrationRoutes();
      const res = (await registerLinearWebhookRoute.handler(
        makeCtx({ params: { projectId: 'p1' }, body: { url: 'https://x' } }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(400);
      expect((res.body as { error: { code: string } }).error.code).toBe('NO_LINEAR_TOKEN');
    });

    it('saves webhook id/url on success', async () => {
      seedProject({ id: 'p1', linear_token: 't', linear_team_id: 'team-1' });
      const { registerLinearWebhookRoute } = await loadIntegrationRoutes();
      const res = (await registerLinearWebhookRoute.handler(
        makeCtx({ params: { projectId: 'p1' }, body: { url: 'https://example.com/wh' } }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(200);
      expect(store.projects.get('p1')!.linear_webhook_id).toBe('wh-1');
      expect(store.projects.get('p1')!.linear_webhook_url).toBe('https://example.com/wh');
    });
  });

  describe('POST /projects/:projectId/settings/github/test', () => {
    it('returns 400 when token is missing', async () => {
      seedProject({ id: 'p1' });
      const { testGithubTokenRoute } = await loadIntegrationRoutes();
      const res = (await testGithubTokenRoute.handler(
        makeCtx({ params: { projectId: 'p1' }, body: {} }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(400);
    });

    it('returns 401 when GitHub API rejects the token', async () => {
      seedProject({ id: 'p1' });
      ...
      expect(res.status).toBe(401);
      expect((res.body as { error: { code: string } }).error.code).toBe('INVALID_TOKEN');
    });

    it('saves token + owner/repo on success', async () => {
      seedProject({ id: 'p1' });
      const { testGithubTokenRoute } = await loadIntegrationRoutes();
      const res = (await testGithubTokenRoute.handler(
        makeCtx({
          params: { projectId: 'p1' },
          body: { token: 'ghp_xxx', owner: 'octo', repo: 'cat' },
        }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(200);
      const row = store.projects.get('p1')!;
      expect(row.github_token).toBe('ghp_xxx');
      expect(row.github_repo_owner).toBe('octo');
      expect(row.github_repo_name).toBe('cat');
    });
  ...
  describe('POST /projects/:projectId/settings/slack/test', () => {
    it('returns 400 when webhookUrl is missing', async () => {
      seedProject({ id: 'p1' });
      const { testSlackWebhookRoute } = await loadIntegrationRoutes();
      const res = (await testSlackWebhookRoute.handler(
        makeCtx({ params: { projectId: 'p1' }, body: {} }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(400);
    });

    it('returns 401 when Slack rejects the webhook', async () => {
      seedProject({ id: 'p1' });
      ...
      const res = (await testSlackWebhookRoute.handler(
        makeCtx({
          params: { projectId: 'p1' },
          body: { webhookUrl: 'https://hooks.slack.com/bad' },
        }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(401);
    });

    it('saves slack_enabled + webhook url + channel on success', async () => {
      seedProject({ id: 'p1' });
      const { testSlackWebhookRoute } = await loadIntegrationRoutes();
      const res = (await testSlackWebhookRoute.handler(
        makeCtx({
          params: { projectId: 'p1' },
          body: { webhookUrl: 'https://hooks.slack.com/xxx', channelId: 'C123' },
        }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(200);
      const row = store.projects.get('p1')!;
      expect(row.slack_enabled).toBe(1);
      expect(row.slack_webhook_url).toBe('https://hooks.slack.com/xxx');
      expect(row.slack_channel_id).toBe('C123');
    });
  });

  describe('GET /projects/:projectId/settings/integrations', () => {
    it('returns 404 when project not found', async () => {
      const { getIntegrationSettingsRoute } = await loadIntegrationRoutes();
      const res = (await getIntegrationSettingsRoute.handler(
        makeCtx({ params: { projectId: 'ghost' } }),
      )) as unknown as JsonResponse;
      expect(res.status).toBe(404);
    });

    it('returns integration status with webhookUrl redacted', async () => {
      seedProject({
        id: 'p1',
        linear_token: 'lt',
        linear_team_id: 'tm',
      ...
      const body = res.body as {
        data: {
          ...
          github: { configured: boolean; owner: string };
          slack: { configured: boolean; webhookUrl: string };
        };
      };
      expect(body.data.linear.configured).toBe(true);
      expect(body.data.github.configured).toBe(true);
      expect(body.data.github.owner).toBe('o');
      expect(body.data.slack.configured).toBe(true);
      // Never return full webhook URL
      expect(body.data.slack.webhookUrl).toBe('****');
    });
  });
});
